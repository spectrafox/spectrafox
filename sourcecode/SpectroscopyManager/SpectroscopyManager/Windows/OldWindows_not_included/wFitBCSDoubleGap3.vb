...
''' </summary>
''' <remarks></remarks>
Public Class wFitBCSDoubleGap3
    Inherits wFormBase
...
#Region "Objects used to fit, such as Fit-Class and SpectroscopyTable"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private WithEvents Fit As cSimplexFit
    ...
    Private Property BroadeningType As cFitFunction_BCSBase.Broadening
        ...
        Set(value As cFitFunction_BCSBase.Broadening)
            ...
            With Me.cboBroadeningType
                Dim SelectedBroadening As cFitFunction_BCSBase.Broadening = DirectCast(.SelectedItem, KeyValuePair(Of String, cFitFunction_BCSBase.Broadening)).Value
                If SelectedBroadening <> value Then
                    For i As Integer = 0 To .Items.Count - 1 Step 1
                        If DirectCast(.Items(i), KeyValuePair(Of String, cFitFunction_BCSBase.Broadening)).Value = value Then
                            .SelectionStart = i
                        End If
                    Next
                End If
                ...
                ' Broadening method:
                If SelectedBroadening <> cFitFunction_BCSBase.Broadening.None Then
                    If SelectedBroadening <> cFitFunction_BCSBase.Broadening.ImaginaryDamping Then
...
#Region "Initialization of the Spectroscopy-Table, and applying the last settings"
    ...
    Private Sub wFitTest_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        With Me.cboBroadeningType
            .Items.Clear()
            .Items.Add(New KeyValuePair(Of String, cFitFunction_BCSBase.Broadening)(My.Resources.BCSFitting_Broad_None, cFitFunction_BCSBase.Broadening.None))
            .Items.Add(New KeyValuePair(Of String, cFitFunction_BCSBase.Broadening)(My.Resources.BCSFitting_Broad_ImaginaryDamping, cFitFunction_BCSBase.Broadening.ImaginaryDamping))
            .Items.Add(New KeyValuePair(Of String, cFitFunction_BCSBase.Broadening)(My.Resources.BCSFitting_Broad_Gauss, cFitFunction_BCSBase.Broadening.Gauss))
            .Items.Add(New KeyValuePair(Of String, cFitFunction_BCSBase.Broadening)(My.Resources.BCSFitting_Broad_Lorentz, cFitFunction_BCSBase.Broadening.Lorentz))
            .ValueMember = "Value"
            .DisplayMember = "Key"
            .SelectedIndex = 1
        End With
        ...
        With Me.cboFitDataType
            .Items.Clear()
            .Items.Add(New KeyValuePair(Of String, cFitFunction_BCSBase.FitFunctionType)(My.Resources.BCSFitting_FitFunctionType_I, cFitFunction_BCSBase.FitFunctionType.I))
            .Items.Add(New KeyValuePair(Of String, cFitFunction_BCSBase.FitFunctionType)(My.Resources.BCSFitting_FitFunctionType_dIdV, cFitFunction_BCSBase.FitFunctionType.dIdV))
            .ValueMember = "Value"
            .DisplayMember = "Key"
            .SelectedIndex = 1
        End With
...
#Region "Fit Procedure"
    ...
    ''' </summary>
    Private Sub ReportFunction(ByVal FitParameters As Double(), Chi2 As Double) Handles Fit.FitEcho
        Dim sb As New System.Text.StringBuilder
        sb.Append("|")
        For i As Integer = 0 To FitParameters.Length - 1 Step 1
            sb.Append("| ")
            sb.Append(FitParameters(i).ToString("N9"))
            If i < FitParameters.Length - 1 Then
                sb.Append(" ")
            Else
                sb.Append(" ||")
            End If
        Next
        sb.Append(" >> ")
        sb.Append(Chi2.ToString("N9"))
        sb.Append(" << ")

        ' Paint new preview
        ' Set current fit parameter to the final fitted set of parameters
        _Delta_Sample1 = FitParameters(0)
        _Delta_Sample2 = FitParameters(1)
        _BroadeningWidth = FitParameters(2)
        _RatioSample1ToSample2 = FitParameters(3)
        _GlobalXOffset = FitParameters(4)
        _GlobalYOffset = FitParameters(5)
        _GlobalYStretch = FitParameters(6)
        _ImaginaryDamping = FitParameters(7)

        HandleFitEcho(sb.ToString, True)
    End Sub
    ...
    Private Sub StartFitting()
        ...
        ' Parameters
        Dim FitParameters() As Double = New Double() {Delta_Sample1,
                                                      Delta_Sample2,
                                                      BroadeningWidth,
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub FitFinished(FitStopReason As cSimplexFit.FitStopReason,
                            FinalParameters As Double(),
        ...
        Select Case FitStopReason
            Case cSimplexFit.FitStopReason.FitConverged
                HandleFitEcho("Fit-Stop-Reason: relative function improvement is no more than: " & Me.Fit.MinimumDeltaChi2)
            Case cSimplexFit.FitStopReason.MaxIterationsReached
                HandleFitEcho("Fit-Stop-Reason: Maximum iterations were taken: " & Me.Fit.MaximumIterations.ToString("N0"))
            Case cSimplexFit.FitStopReason.UserAborted
        ...
        HandleFitEcho("Temperature:      " & T_sample.ToString(TextBoxFormat))
        HandleFitEcho("Delta Tip:        " & Delta_Tip.ToString(TextBoxFormat))
        HandleFitEcho("Delta Sample 1:   " & FinalParameters(0).ToString(TextBoxFormat))
        HandleFitEcho("Delta Sample 2:   " & FinalParameters(1).ToString(TextBoxFormat))
        HandleFitEcho("Broadening Width: " & FinalParameters(2).ToString(TextBoxFormat))
        HandleFitEcho("Broadening Type:  " & BroadeningType.ToString)
        HandleFitEcho("ImaginaryDamping: " & FinalParameters(7).ToString(TextBoxFormat))
        HandleFitEcho("Delta1 / Delta2:  " & FinalParameters(3).ToString(TextBoxFormat))
        HandleFitEcho("Global X Offset:  " & FinalParameters(4).ToString(TextBoxFormat))
        HandleFitEcho("Global Y Offset:  " & FinalParameters(5).ToString(TextBoxFormat))
        HandleFitEcho("Global Y Stretch: " & FinalParameters(6).ToString(TextBoxFormat))
        HandleFitEcho("------------------")
        ...
        ' Set current fit parameter to the final fitted set of parameters
        _Delta_Sample1 = FinalParameters(0)
        _Delta_Sample2 = FinalParameters(1)
        _BroadeningWidth = FinalParameters(2)
        _RatioSample1ToSample2 = FinalParameters(3)
        _GlobalXOffset = FinalParameters(4)
        _GlobalYOffset = FinalParameters(5)
        _GlobalYStretch = FinalParameters(6)
        _ImaginaryDamping = FinalParameters(7)

        SetButtonsToFitModus(False)

        ' If the Fit-Operation was successfull, show final set of data!
        If FitStopReason <> cSimplexFit.FitStopReason.UserAborted Then
...
#Region "Fit-Echo"
    ...
    ''' <remarks></remarks>
    Private Delegate Sub _HandleFitEcho(ByRef message As String, ByVal UpdatePreview As Boolean)
    Private Sub HandleFitEcho(ByRef Message As String, Optional ByVal UpdatePreview As Boolean = False)
        If Me.FitEcho.InvokeRequired Then
            Dim _delegate As New _HandleFitEcho(AddressOf HandleFitEcho)
            Invoke(_delegate, Message, UpdatePreview)
        Else
